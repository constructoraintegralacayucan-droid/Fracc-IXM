...
export default async function DesarrolloPage({
  ...
  const [precioDesde, imagenes, testimonios] = await Promise.all([
    getPrecioDesde(config.id),
    getGaleriaImagenes(config.id),
    getTestimoniosAprobados(config.id),
  ]);
